...
export class AddGroupUserComponent implements OnInit {
    ...
    this.form = this.formBuilder.group(
      {
        ...
        contact_name: ['', [Validators.required, Validators.minLength(6)]],
        group_user_type: ['', [Validators.required, Validators.minLength(1)]],
  ...
  bindData() {
    ...
    this.form.patchValue({
      ...
      group_user_type: this.editDataModel?.group_user_type,
  ...
  onSubmit(): void {
    ...
    if (this.isEdit == false) {
      this.httpService.postWithAuth(AppConstants.CREATE_GROUP_USER, JSON.stringify(this.form.value, null, 2)).subscribe(
        (response: any) => {
      ...
      this.httpService.patch(AppConstants.UPDATE_GROUP_USER, JSON.stringify(this.form.value, null, 2)).subscribe(
        (response: any) => {
          this.showSuccessMessage(response);
        },
        error => {
